use nalgebra::{SVector, OMatrix, Dyn};
...
impl<const DIM: usize> Curve<DIM> for SecondOrderEdge<DIM> {
  ...
  fn get_vertex_at(&self, t: f64) -> Vertex<DIM> {
      return self.start_vertex * SecondOrderEdge::<DIM>::interpol1(t) + self.middle_vertex * SecondOrderEdge::<DIM>::interpol2(t) + self.end_vertex * SecondOrderEdge::<DIM>::interpol3(t);
  }
  fn is_unbounded(&self) -> bool {
      false
  }
...
impl<const DIM: usize> Face<DIM> for StraightQuadrilaterial<DIM> {
  ...
  fn get_boundary_curves(&self) -> Vec<Box<dyn Curve<DIM>>> {
      ...
      result.push(Box::new(StraightEdge::new(self.vertices[0],self.vertices[1])));
      result.push(Box::new(StraightEdge::new(self.vertices[1],self.vertices[2])));
      result.push(Box::new(StraightEdge::new(self.vertices[2],self.vertices[3])));
      result.push(Box::new(StraightEdge::new(self.vertices[3],self.vertices[0])));
...
trait Chart<const INPUT_DIM: usize, const OUTPUT_DIM: usize> {

  type Output;
  type Input;

  fn map_reference_to_actual(input_ref: Self::Input) -> Self::Output; 
}