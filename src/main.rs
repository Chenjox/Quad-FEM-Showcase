...
fn shape_functions_2d_grad(index: usize,xi_1: f64, xi_2: f64) -> OMatrix<f64, Const<2>, Const<1>> {
    return OMatrix::<f64, Const<2>, Const<1>>::new(g2_dxi1_vec(xi_1, xi_2)[index], g2_dxi2_vec(xi_1, xi_2)[index]);
}
...
fn main() {
    let m = get_FE_mesh(30.0, 3.0, 10, 3);
    ...
    for (elem_index, elem) in m.elements.column_iter().enumerate() {
        ...
        for gauss_point in gauss.column_iter() {
            ...
            for (local_index, point_index) in elem.iter().enumerate() {
                ...
                let gradient = OMatrix::<f64, Const<2>, Const<1>>::new(shape_deriv_1[local_index], shape_deriv_2[local_index]);
            ...
            let shape_actual = g2_vec(xi_1, xi_2);
            // Jetzt brauchen wir die Ableitungen nach den tatsächlichen Koordinaten.
            for (local_index, point_index) in elem.iter().enumerate() {
                let point_index = *point_index;
                let coords = m.get_point(point_index);

                let shape_deriv = inv_jacobian * shape_functions_2d_grad(local_index, xi_1, xi_2);

                println!("{:2.2}",shape_deriv);
